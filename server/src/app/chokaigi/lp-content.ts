...
export const LP_DESCRIPTION =
  "ニコニコ超会議で生まれるみんなのすれ違いが、やがて日本中にひろがり、クリエイター同士が応援しあえる——そんなやさしいつながりを、ひとことずつ支えたい企画の予告ページです。小さな応援のクリエイターが星屑のように集い、ひとつの超会議という光となって放物線のように遠くまで届くイメージを大切にしています。会場では匿名で短くつなぎ、迷子になりにくい案内も用意します。近接マッチングには参加中の位置情報（一定間隔の送信・バックグラウンド更新を含む）が必要です。りんく・こん太・たぬ姉の3人がゆっくりガイドします。";

export const HERO_HEADING = "超会議で、すれ違った人とひとこと";

export const HERO_LEAD =
  "ニコニコ超会議で出会うみんなの「すれ違い」が、やがて日本中にひろがり、クリエイターみんなが応援しあえる。そんな優しい世界を、ひとことずつ築いていけたら——それを願って、この企画を組んでいます。\n\nひとりひとりの小さな応援するクリエイターは、星屑のようにちりばみ、ニコニコ超会議というひとつの「光」にあつまり、放物線を描くような巨大なひかりになっていく。そんなイメージで、画面の向こうの応援も、会場の空気も、つながりとして描いています。\n\nすれ違い検出には、会期中に端末から位置情報を継続的に送る設計が前提です（秒単位のトラッキングではなく、マッチング用の間欠送信・既存アプリと同様の扱いを想定）。会場が広くてもエリアを頼りに迷子になりにくくし、同じニコニコユーザー同士が匿名でひとこと交信できる導線を整えます。超会議のあとも、全国どこからでもゆるくつながれる発展を視野に入れて設計します。";
...
export const VENUE_SECTION_INTRO =
  "幕張メッセ周辺は、展示場やイベントホールが続き、まるで超巨大な迷路のように広い会場です。公式の会場案内・掲示・目印を頼りに、行きたいブースへの回るルートをざっくり決めておくと、迷子になりにくくて効率的です。超会議では、すれ違う相手を尊重し、短いやりとりのなかでお互いのいいところに気づき、応援の気持ち（いいねのイメージ）を重ねあえたらうれしい——そんな空気がひろがるとよいな、という想いも込めています。ブースやステージを回るクリエイター同士のすれ違いも含め、同じ空気の中にいるニコニコユーザーと、粗いエリア表示とすれ違いマッチで、負担の少ない形でつながれることを目指します。";
...
export const HOME_HERO_LEAD =
  "ニコニコ超会議でのみんなのすれ違いが、やがて日本中にひろがり、クリエイター同士が応援しあえる——そんな優しいつながりを、匿名で短くつなぐアプリ「すれちがいライト」から支えたい、という案内です。小さな応援が星屑のように集まり、ひとつの光になるイメージで語っています。企画予告（/chokaigi）では、りんく・こん太・たぬ姉がゆっくりガイドします。";

/** Auth-less First: 主要体験は未ログインで通す（登録・位置送信はログイン後） */
export const AUTH_LESS_FIRST_COPY =
  "ログインなしで検索・会場ガイド・ライブマップ閲覧まで試せます。参加登録や位置の送信はログイン後でOKです。";

/** ルート / の短い要約（詳細は /chokaigi へ） */
export const HOME_USAGE_SECTION_TITLE = "すれ違い通信のひとこと";

export const HOME_USAGE_SECTION_INTRO =
  "会場ですれ違ったニコニコユーザーと、匿名で短いメッセージを交わすイメージです。その積み重ねが、やがて日本中のクリエイター同士の応援につながったらうれしい、という気持ちを込めています。細部は公開版に合わせて更新します。";
...
export const JAPAN_LOCATOR_CAPTION =
  "ニコニコ超会議の会場は千葉県千葉市美浜区の幕張メッセ周辺です。下の②公式PDF・①概略図で館内の位置関係を確認してください。";
...
export const JAPAN_LOCATOR_MAP_LINK_LABEL = "幕張メッセ周辺を地図で開く";

/** 都道府県名一覧（地図下の参考リスト・JIS 都道府県コード順） */
export const JAPAN_LOCATOR_PREF_HEADING = "都道府県名（参考・コード順）";

export const JAPAN_PREFECTURE_NAMES = [
  "北海道",
  "青森県",
  "岩手県",
  "宮城県",
