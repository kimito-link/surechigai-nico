...
/**
 * ─────────────────────────────────────────────────
 *  超会議 2026 ヒーロー：ファーストビュー v3
 *  「ゆっくり解説 × すれ違い通信」を同格の 2 本柱で見せる。
 *  星野ロミさん的な "誰でも誰かを紹介できる文化" を継ぎつつ、
 *  企画の核は "会場で近くの人と出会えるすれ違い通信"。
 * ─────────────────────────────────────────────────
 */

/** ヒーローのメインタイトル（2 本柱を「×」で並列） */
export const HERO_TITLE_MAIN = "すれちがいライト × ゆっくり解説";

/** ヒーローのサブコピー（ひと言で企画を要約） */
export const HERO_TITLE_SUB =
  "超会議 2026 ― 誰かを紹介する、近くの人と出会う。";

/** 柱①：ゆっくり解説（= もう一つの名刺） */
export const HERO_PILLAR_YUKKURI = {
  title: "ゆっくり解説",
  sub: "誰でも紹介できる即席の名刺",
  body:
    "X ID を入れるだけで、りんく・こん太・たぬ姉の 3 人がその人を即席で紹介します。ログインなしで試せて、解説は専用 URL に保存されます。自分を紹介しても、推しを紹介しても OK。",
} as const;

/** 柱②：すれ違い通信（= 企画の核） */
export const HERO_PILLAR_SURECHIGAI = {
  title: "すれ違い通信",
  sub: "現地・全国で近くの人と出会う",
  body:
    "X でログインして位置を交換。会場では近くの人とマッチ、全国ではゆるい距離感でつながれます。オフ会・いいねの種になります。",
  bullets: [
    "会期中は粗い位置を間欠送信（秒単位で追いません）",
    "参加県は任意公開・デフォルト非公開（会場で自己紹介の種に）",
    "連絡は X アカウント同士で（DM は非搭載）",
  ],
  ctaGuest: "X でログインして参加",
  ctaSignedIn: "ダッシュボードへ",
} as const;

/** シナジー文（2 本柱の下）: 柱①と②がどう噛み合うかを 3 行で伝える */
export const HERO_SYNERGY_LEAD =
  "2 つはつながっていて、見せたい分だけ見せられる。";

export const HERO_SYNERGY_DETAIL =
  "解説された人が会場にいたら、すれ違いで見つかる。すれ違った人を、その場でゆっくり解説で読める。";

export const HERO_SYNERGY_PRIVACY_NOTE =
  "「〇〇から来たんです」の自己紹介も、見せたい人にだけ（参加県は任意公開・デフォルト非公開）。";

/**
 * 解説完了オーバーレイで出すシナジー誘導文。
 * `{handle}` は描画時に実際の X ハンドルに置換する（プレースホルダ契約）。
 */
export const HERO_POST_EXPLAIN_SYNERGY =
  "💡 @{handle} さんがすれ違い参加中なら、会場で見つかるかも。自分も参加すると近づけます。";
